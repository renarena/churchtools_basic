...
function home_main() {
  ...
  if (!empty($config["admin_message"])) addErrorMessage($config["admin_message"]);
  
  checkFilesDir();
  
  $btns = churchcore_getModulesSorted();
  
  if (!empty($_SESSION["family"])) addInfoMessage(t('there.are.more.users.with.the.same.email'));
  ...
  foreach ($btns as $key) {
    if ((isset($config[$key. "_startbutton"]))&& ($config[$key. "_startbutton"]== "1")&& (user_access("view", $key))) $txt .= '<a class="btn btn-large" href="?q='.
         $key. '">'. $config[$key. "_name"]. '</a>&nbsp;';
  }
  $txt .= '</p>';
  $txt .= '</div>';
...
function home__memberlist_saveSettings($form) {
  if (isset($_POST["btn_1"])) {
    ...
    foreach ($form->fields as $key => $value) {
      db_query("insert into {cc_config} (name, value) values (:name,:value) on duplicate key update value=:value", array (
                                                                                                                          ":name" => $key, 
                                                                                                                          ":value" => $value
      ));
    }
