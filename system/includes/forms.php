...
class CC_Field extends CC_HTMLElement {
  ...
  public function setValue($val) {
    if ($this->fieldType== "CHECKBOX") {
      if (($val== "on")|| ($val== 1)) $this->value = true;
  ...
  public function render() {
    ...
    if (($this->fieldType== "INPUT_REQUIRED")|| ($this->fieldType== "INPUT_OPTIONAL")|| ($this->fieldType== "TEXTAREA")||
      ...
      if ($this->error!= null) $txt .= " error";
      
      $txt .= '">';
      
      if ($this->fieldType== "TEXTAREA") {
        $txt .= '<textarea rows=6 class="span12" name="'. $this->form->getName(). '['. $this->getName(). ']" id="'.
        ...
        $txt .= '<input class="span3" name="'. $this->form->getName(). '['. $this->getName(). ']" id="'.
             $this->form->getName(). '_'. $this->getName(). '" ';
        if ($this->fieldType== "PASSWORD") $txt .= 'type="password" ';
        else $txt .= 'type="text" ';
      ...
      if (($this->error!= null)) $txt .= '<span class="help-inline error">'. $this->error. '</span>';
      
      $txt .= '</div>';
    }
    else if ($this->fieldType== "CHECKBOX") {
      $txt .= '<label class="checkbox" for="'. $this->form->getName(). '_'. $this->getName(). '">';
      $txt .= '<input name="'. $this->form->getName(). '['. $this->getName(). ']" id="'. $this->form->getName(). '_'.
           $this->getName();
      $txt .= '" type="checkbox"';
      ...
      return "FieldType $this->fieldType not implemented!";
    return $txt;
  }
...
class CC_Model {
  ...
  public function addField($name, $class, $fieldType, $label = "", $autofocus = false) {
    if (!in_array($fieldType, $this->FieldTypes)) echo ("FieldTyp $fieldType nicht vorhanden!");
  ...
  public function render() {
    ...
    if (isset($_POST[$this->getName()])) {
      ...
      if ($isValid) {
        if ($this->validator== null) return "no validator given!";
    ...
    if ($this->header_big!= null) $txt .= "<h1>$this->header_big</h1>";
    if ($this->header_small!= null) $txt .= "<p>$this->header_small</p>";
    ...
    foreach ($this->fields as $field) {
      $txt .= $field->render();
      if ($field->isRequired()) $one_required = true;
    }
    ...
    if ($one_required) $txt .= '<p class="note">'. t("fields.with.asterisk.has.to.be.filled"). '</p>';
